# GradioAI FrontEnd & BackEnd Integration specifications

This document outlines the architecture, communication interfaces, database synchronizations, and execution pathways that link the GradioAI React FrontEnd and FastAPI BackEnd applications.

---

## 1. Overview of the Integration Architecture

GradioAI utilizes an **indirect, database-centric integration model** supplemented by **serverless Edge Functions** acting as smart API gateways. 
...
The instructor clicks "Evaluate" on a submission in [AssignmentDetailPage.tsx](file:///e:/GradioAI/FrontEnd/src/pages/AssignmentDetailPage.tsx). This executes the `useEvaluateSubmission` mutation inside [useData.ts](file:///e:/GradioAI/FrontEnd/src/hooks/useData.ts):
```typescript
...
The serverless Edge Function [evaluate/index.ts](file:///e:/GradioAI/FrontEnd/supabase/functions/evaluate/index.ts) receives the request, sets the submission's status to `'evaluating'` to prevent concurrent triggers, and evaluates the dispatch mode:
...
The FastAPI backend receives the request on the `/evaluate` endpoint defined in [evaluate.py](file:///e:/GradioAI/BackEnd/src/routes/evaluate.py). It registers a new task with the [job_manager.py](file:///e:/GradioAI/BackEnd/src/services/job_manager.py) and launches the task asynchronously using FastAPI's `BackgroundTasks` wrapper. The backend immediately returns a `202 Accepted` response containing:
```json
{
...
The backend worker [pipeline_worker.py](file:///e:/GradioAI/BackEnd/src/workers/pipeline_worker.py) executes the LangGraph `GradingSystem` pipeline:
...
3.  **AI Text Ingestion:** The Edge Function [parse-pdf/index.ts](file:///e:/GradioAI/FrontEnd/supabase/parse-pdf/index.ts) reads the file stream, converts the PDF array buffer to base64, and prompts `google/gemini-2.5-flash` to extract all raw text.
...
The FrontEnd type declarations in [database.ts](file:///e:/GradioAI/FrontEnd/src/types/database.ts) map to the database payloads generated by the BackEnd in [result.py](file:///e:/GradioAI/BackEnd/src/mapping/result.py):
