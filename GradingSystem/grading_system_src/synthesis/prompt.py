...
def invoke_llm_synthesis(
    llm: Any,
    system_prompt: str,
    user_msg: str,
    manuscript_path: str | Path | None,
) -> Any:
    """Invokes the LLM using the PDF file content if available.
    Falls back to text-only prompt if the PDF invocation fails.
    """
    if manuscript_path:
        path = Path(manuscript_path)
        if path.exists() and path.suffix.lower() == ".pdf":
            try:
                logger.info("Attempting primary PDF-based evaluation for %s", path.name)
                ...
                if pdf_bytes:
                    ...
                    logger.info("Primary PDF-based evaluation succeeded.")
                    return response
            except Exception as e:
                logger.warning(
                    "Primary PDF-based evaluation failed with error: %s. "
                    "Falling back to text-only evaluation.",
                    e,
                )

    logger.info("Running text-only evaluation.")
...
def generate_review(
    ...
    *,
    ...
    response = invoke_llm_synthesis(
        llm=llm,
        system_prompt=load_prompt("review_synthesis"),
        user_msg=user_msg,
