...
from grading_system_src.exceptions import LLMParseError
from grading_system_src.llm import get_llm, invoke_llm
from grading_system_src.model_cache import get_encoder
    ...
    for i in range(0, len(claims), batch_size):
        ...
        try:
            ...
            if isinstance(data, dict):
                # 1. Check if there's a list somewhere inside the dict (e.g. {"classifications": [...]})
                list_val = None
                ...
                if list_val is not None:
                    for item in list_val:
                        if isinstance(item, dict) and "index" in item and "type" in item:
                            type_map[int(item["index"])] = item["type"]
                else:
                    # 2. Check if it's a map of indices to strings/dicts (e.g. {"0": "SYNTHESIS"} or {"0": {"type": "SYNTHESIS"}})
