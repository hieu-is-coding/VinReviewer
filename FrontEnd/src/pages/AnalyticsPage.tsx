...
const AnalyticsPage = () => {
  const { data, isLoading, rawEvaluations, rawSubmissions } = useAnalyticsData();
  ...
  const radarData = data.criteriaBreakdown.map((c) => ({ criterion: c.name, score: c.avgPct, fullMark: 100 }));
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-fade-in pb-12">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Comprehensive insights across {data.totalEvaluations} evaluations</p>
          </div>
          <button
        ...
        {expandedSections.classes !== false && data.classPerformance.length > 0 && (
          <div className="bg-card rounded-lg shadow-card border border-border p-5">
